...
function StopTracks(originId){
    ...
    for(var i = 0, len = audios.length; i < len;i++){
            ...
            if (audios[i]) {
                ...
                if(audios[i].parentElement.className != "fa fa-check") {
                    audios[i].parentElement.className = "fa fa-play";
                }
...
function StopAudio(originId) {
    ...
    playButton.className = "fa fa-check";
}
...
function togglePlay(originId){
  ...
  if (playButton.className === "fa fa-pause") {
    paused = true;
